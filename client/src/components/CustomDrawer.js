...
export default function CustomDrawer({ open, onClose, side = 'left', children, className }) {
  ...
  return (
    <div className={` overflow-x-hidden overflow-y-auto  inset-0 z-40 ${open ? 'fixed' : 'hidden'} `}>
      <div className="opacity-80 fixed inset-0 bg-base-300" onClick={onClose} />
      <div className={`bg-base-100 absolute transition-all z-50 overflow-y-auto ${classes} ${className}`} >
